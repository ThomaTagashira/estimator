...
class DeleteEstimateAPITest(TestCase):
    def setUp(self):
        ...
        self.estimate = UserEstimates.objects.create(user=self.user, estimate_id=1)
    ...
    def test_delete_user_estimate(self):
        ...
        self.assertFalse(UserEstimates.objects.filter(estimate_id=1).exists())

    def test_delete_estimate_not_owned(self):
        other_user = User.objects.create_user(username='otheruser', password='otherpassword')
        other_estimate = UserEstimates.objects.create(user=other_user, estimate_id=2)

        response = self.client.delete(f'/api/delete-estimate/{other_estimate.estimate_id}/')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
        self.assertTrue(UserEstimates.objects.filter(estimate_id=2).exists())
